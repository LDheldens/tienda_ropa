...
class BoxForm(ModelForm):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.initial['cash_sale'] = Box.total_efectivo() + Box.total_payments_ctas_collect_efectivo()
        # self.initial['cash_credit'] = Box.total_credito()
        self.initial['sale_card'] = Box.total_tarjeta_yape_plin() + Box.total_payments_ctas_collect_credito()
        self.initial['box_final'] = Box.total_efectivo() + Box.total_payments_ctas_collect_efectivo() + Box.total_tarjeta_yape_plin() + Box.total_payments_ctas_collect_credito()
    class Meta:
        model = Box
        fields = '__all__'
        widgets = {
            ...
            'hours_close': forms.TimeInput(format='%H:%M', attrs={'type': 'time'}),
