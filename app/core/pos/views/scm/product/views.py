                ...
                if archive:
                    ...
                    with transaction.atomic():
                        ...
                        for row in range(2, excel.max_row + 1):
                            ...
                            print('nombre: ',name)
                            description = excel.cell(row=row, column=4).value
                            stock = excel.cell(row=row, column=5).value
                            if stock is not None:
                                stock = int(stock)
                            else:
                                stock = 0
                            price = excel.cell(row=row, column=6).value
                            if price is not None:
                                price = float(price)
                            else:
                                price = 0.0
                            pvp = excel.cell(row=row, column=7).value
                            if pvp is not None:
                                pvp = float(pvp)
                            else:
                                pvp = 0.0
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            for product in Product.objects.filter().order_by('id'):
                ...
                worksheet.write(row, 3, product.desc, row_format)
                worksheet.write(row, 4, product.stock, row_format)
